...
class PredictLitho:
    ...
    def train(self, target, start, end, plot, model="RF", CV=2):
        ...
        try:
            if CV < 2:
                raise ValueError(
                    f"Number of cross validation folds should be greater than 2; {CV} specified"
                )

        except ValueError as err:
            print(err)

        train_features, train_target, test_features = self._preprocess(
            df, target, start, end
        )
        ...
        X_train, X_test, y_train, y_test = ms.train_test_split(
            train_features, train_target, test_size=0.2, random_state=20
        )

        if model == "RF":

            model1 = RandomForestRegressor(
                n_estimators=100, max_depth=6, random_state=20, verbose=2
            )

        elif model == "XGB":
            model1 = xgb.XGBRegressor(
                n_estimators=3000, max_depth=6, reg_lambda=300, random_state=20
            )
    ...
    def predict(self, target, start, end, model="RF", CV=2):
        ...
        trained_model, test_features = self.train(
            target, start, end, self.plot, model=model, CV=CV
        )
...
class PredictLabels:
    ...
    def _preprocess(self, df):
        ...
        group_encoded = pickle.load(
            open(os.path.join(file_dir + "/model/group_encoded"), "rb")
        )
        formation_encoded = pickle.load(
            open(os.path.join(file_dir + "/model/eormation_encoded"), "rb")
        )
    ...
    def pretrain(self):
        ...
        for i in range(1, 3):
            model = xgb.Booster()
            model.load_model(
                os.path.join(file_dir + "/model/lithofacies_model" + str(i) + ".model")
            )
    ...
    def plot_lithofacies(
        ...
        self.log1, self.log2, self.log3, self.log4, self.log5 = (
            log1,
            log2,
            log3,
            log4,
...
class DataHandlers:
    ...
    def encode_categorical(self):
        ...
        for column in cat_df.columns:
            ...
            if (
                check_cardinality(cat_df, column) == "Unique"
                or check_cardinality(cat_df, column) == "Distinct"
            ):
    ...
    def set_mnemonics(
        ...
        self.GR, self.RHOB, self.NPHI, self.CALI, self.BS, self.RDEP = (
            GR,
            RHOB,
            NPHI,
            CALI,
        ...
        self.RMED, self.RSHA, self.PEF, self.DTC, self.SP, self.ROP, self.DTS = (
            RMED,
            RSHA,
            PEF,
            DTC,
        ...
        self.DCAL, self.DRHO, self.MUDWEIGHT, self.RMIC, self.ROPA = (
            DCAL,
            DRHO,
            MUDWEIGHT,
            RMIC,
        ...
        self.RXO, self.GROUP, self.FORMATION, self.X_LOC, self.Y_LOC, self.Z_LOC = (
            RXO,
            GROUP,
            FORMATION,
            X_LOC,
        ...
        new_df = pd.DataFrame()
        new_df["WELL"], new_df["DEPTH_MD"], new_df["X_LOC"] = (
            df[WELL],
            df[DEPTH_MD],
            df[X_LOC],
        )
        new_df["Y_LOC"], new_df["Z_LOC"], new_df["GROUP"] = (
            df[Y_LOC],
            df[Z_LOC],
            df[GROUP],
        )
        new_df["FORMATION"], new_df["CALI"], new_df["RSHA"] = (
            df[FORMATION],
            df[CALI],
            df[RSHA],
        )
        ...
        new_df["DCAL"], new_df["DRHO"], new_df["MUDWEIGHT"], new_df["RMIC"] = (
            df[DCAL],
            df[DRHO],
            df[MUDWEIGHT],
            df[RMIC],
