...
def make_facies_log_plot(logs, x1, x2, x3, x4, x5, Depth=False):
    ...
    cmap_facies = colors.ListedColormap(facies_colors[0:no], "indexed")
    ...
    im = ax[5].imshow(
        cluster, interpolation="none", aspect="auto", cmap=cmap_facies, vmin=0, vmax=12
    )
...
def compare_plots(logs, x1, x2, x3, x4, x5, Depth=False):
    ...
    ax[4].plot(logs[x5], logs.Depth, "-", color="black")
    im = ax[5].imshow(
        cluster1, interpolation="none", aspect="auto", cmap=cmap_facies, vmin=0, vmax=12
    )
    im = ax[6].imshow(
        cluster2, interpolation="none", aspect="auto", cmap=cmap_facies, vmin=0, vmax=12
    )
